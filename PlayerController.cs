...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        if (gamdir.GetFireOrder() && liveordie)//ゲームスタート中と生きてる時だけ操作できる
        {
    ...
    IEnumerator DamageShake()//ダメージ演出
    {
    ...
    IEnumerator InvincibleTime()//無敵時間
    {
        invincibling = true;
    ...
    public void CoverShaking()//カバー揺らし
    {
        StartCoroutine(CoverShaker());

    }
    IEnumerator CoverShaker()//カバー揺らす処理
    {
    ...
    public void HammerCockingWhenShots()//射撃状態のハンマーコック
    {
        RevolverWheeling();
        RevolverWheeling();
        cocked = true;
    ...
    public void RevolverWheelingEnd()//リボルバー回転終わり
    {
        RevolverWheeling();
        CartridgeVisualizing();
    ...
    void EmptyChecker()//弾が一発も入ってないか確認する
    {
        emptychecker = 0;
        for (int i = 0; i < revolverstate.Length; i++)
        {
    ...
    void HitFlashIgnition()//着弾エフェクト表示
    {
    ...
    public void ExtractionCartridge()//弾を取り出す
    {
    ...
    public void LoadingCartridge()//装填
    {
    ...
    void CartridgeVisualizing()//配列のシリンダーに合わせて画像を表示
    {
        for (int i = 0; i < revolverstate.Length; i++)
        {
