...
public class EnemyController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (currentTime > randomshotspan && enemylife > 0)  //撃つまでの時間がたまったか
        {
            if (!gatringgun) randomshotspan = Random.Range(0.6f, 1);

            actiondice = Random.Range(1, 7);//行動決める
            if (actiondice > 2 && ammocapacity > 0 && !covering)//1,2の場合確定ヒット処理
            {
            ...
            else if (ammocapacity == 0 && !reloading && !actioning)//弾切れならリロード
            {
                StartCoroutine(EnemyReloading());
                if (covertype) EnemyCovering();
                reloading = true;
            }
            else if (covertype && actiondice <= 2 && !covering && !reloading && !actioning)//カバーに隠れる
            {
    ...
    void EnemyShowingFace() //カバーから顔出す
    {
        if (coververticalmove)
        {
            transform.DOMoveY(transform.position.y + 30, 0.2f);
    ...
    public void EnemyDamage(int damage) //ダメージ処理
    {
        enemylife -= damage;
